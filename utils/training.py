...
class TorchTrainer():
    ...
    def fit(self, dl_train: DataLoader, dl_val: DataLoader,
        ...
        for epoch in range(num_epochs):
            ...
            # ====== YOUR CODE: ======
            best_loss = -1 if epoch == 0 else best_loss

            train_epoch_results = self.train_epoch(dl_train, **kw)
            train_loss.extend(train_epoch_results.losses)
            train_acc.append(train_epoch_results.accuracy)

            val_epoch_results = self.test_epoch(dl_val, **kw)
            val_loss.extend(val_epoch_results.losses)
            val_acc.append(val_epoch_results.accuracy)

            epochs_without_improvement += 1
            if best_acc < val_epoch_results.accuracy or best_acc == -1:
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        predicted_y = self.model(X) # forward pass
        loss = self.loss_fn(predicted_y, y) # calculate loss
        loss.backward()
        self.optimizer.step()
        num_correct = (predicted_y.argmax(dim=1) == y).sum().item() # number of correct predictions
        loss = loss.item() # return loss as a number
        # raise NotImplementedError()
        # ========================
