...
DATA_DIR = os.path.join(os.path.expanduser('~'), '.pytorch-datasets') #replace  os.getenv('HOME') with os.path.expanduser('~')
...
def run_experiment(run_name, out_dir='./results', seed=None,
    ...
    for i in filters_per_layer:
        for j in range(layers_per_block):
            actual_filters.append(i)
    model = model_cls(in_size = (3,32,32), out_classes = 10, filters = actual_filters, pool_every = pool_every, hidden_dims = hidden_dims)
    loss_func = torch.nn.CrossEntropyLoss()

    optimizer = torch.optim.SGD(model.parameters(), lr = lr)
    # optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=reg) #Was used when L2 regularization was needed
    #optimizer = torch.optim.Adam(model.parameters(), lr=lr) #When SGD was tested, this line was commented out and the line below was used instead
    # optimizer = torch.optim.SGD(model.parameters(), lr=lr, weight_decay=reg, momentum=0.9) #When Adam was tested, this line was commented out and the line above was used instead

    trainer = training.TorchTrainer(model, loss_func, optimizer, device)
    fit_res = trainer.fit(dl_train=dl_train, dl_val=dl_val, num_epochs=epochs, checkpoints=checkpoints, early_stopping=early_stopping, print_every=1, post_epoch_fn=None)
